...
parser.add_argument("-opt", type=str, help="Path to options YMAL file.", default="test_latent.yml")
args = parser.parse_args()
args.opt = os.path.join('src/unet_latent/options/test/', args.opt)
opt = option.parse(args.opt, ts, is_train=False)
...
for test_loader in test_loaders:
    ...
    for i, test_data in enumerate(test_loader):
        ...
        if LQ.shape[1] != 3:
            logger.warning(f'Images should have 3 channels. {img_name} only has {LQ.shape[1]}')
            continue
