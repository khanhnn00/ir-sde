...
import src.unet_latent.options as option
from src.unet_latent.models import create_model
...
def main():
    ...
    parser.add_argument("-opt", type=str, help="Path to option YMAL file.", default="train_sisr.yaml")
    parser.add_argument(
    ...
    args.opt = os.path.join('src/unet_latent/options/train/', args.opt)
        ...
        for _, train_data in enumerate(train_loader):
            ...
            if current_step % opt["train"]["val_freq"] == 0 and rank <= 0:
                ...
                for _, val_data in enumerate(val_loader):
                    ...
                    visuals = model.get_current_batch_visuals()
                    
                    fid_pred = visuals["Output"].detach().float().cpu().clamp_(0, 1)  # clamp
                    fid_pred = (fid_pred - 0) / (1 - 0)  # to range [0,1]
                    
                    fid_gt = visuals["GT"].detach().float().cpu().clamp_(0, 1)  # clamp
                    fid_gt = (fid_gt - 0) / (1 - 0)  # to range [0,1]

                    # calculate PSNR
                    avg_psnr += util.calculate_psnr(output, gt_img)
                    avg_fid += util.calculate_fid(fid_gt, fid_pred)
                    
                    if idx < 11:
                        # visualize
                        h, w, c = gt_img.shape
                        margin_array = np.full((h, 30, c), (255, 255, 255), dtype=gt_img.dtype)
                        merged = np.concatenate((gt_img, margin_array, output), axis=1)
                        merge_tensor = torchvision.transforms.functional.to_tensor(merged)
                        tb_logger.add_image(f"val/{current_step}_{idx}", merge_tensor, global_step=current_step)
                        
                    idx += 1

                avg_psnr = avg_psnr / idx
                avg_fid = avg_fid / (idx * LQ.shape[0])
                ...
                logger.info("# Validation # FID: {:.6f} | Iter: {}".format(avg_fid, current_step))
                ...
                logger_val.info(
                    "<epoch:{:3d}, iter:{:8,d}, fid: {:.6f}".format(
                        epoch, current_step, avg_fid
                    )
                )
                print("<epoch:{:3d}, iter:{:8,d}, psnr: {:.6f}".format(
                        epoch, current_step, avg_psnr
                    ))
                print("<epoch:{:3d}, iter:{:8,d}, fid: {:.6f}".format(
                        epoch, current_step, avg_fid
                    ))
                # tensorboard logger
                if opt["use_tb_logger"] and "debug" not in opt["name"]:
                ...
                if opt["use_tb_logger"] and "debug" not in opt["name"]:
                    tb_logger.add_scalar("fid", avg_fid, current_step)
