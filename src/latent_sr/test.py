...
parser.add_argument("-opt", type=str, required=False, help="Path to options YMAL file.", default="src/latent_sr/options/test/latent_sisr.yml")
opt = option.parse(parser.parse_args().opt, "0", is_train=False)
...
for test_loader in test_loaders:
    ...
    for i, test_data in enumerate(test_loader):
        ...
        h, w, c = GT_.shape
        margin_array = np.full((h, 30, c), (255, 255, 255), dtype=output.dtype)
        merged = np.concatenate((GT_, margin_array, output), axis=1)
        ...
        util.save_img(merged, save_img_path)
