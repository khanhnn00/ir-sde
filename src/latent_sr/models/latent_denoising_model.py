...
from src.utils.img_utils import pca_8ch_to_3ch

logger = logging.getLogger("base")


class LatentDenoisingModel(BaseModel):
    ...
    def test(self, sde=None, hidden=None, perform_ode=False, save_states=False):
        ...
        latent = pca_8ch_to_3ch(latent)

        tvutils.save_image(self.output, f'image/output.png', normalize=False)
        self.latent = tvutils.save_image_and_return(latent, f'image/latent.png', normalize=False)
    ...
    def get_current_batch_visuals(self, need_GT=True):
        out_dict = OrderedDict()
        out_dict["Input"] = self.LQ.detach().float().cpu()
        out_dict["Output"] = self.fake_gt.detach().float().cpu()
        if need_GT:
