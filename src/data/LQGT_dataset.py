...
class LQGTDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.opt["phase"] == "train":
            H, W, C = img_LR.shape
            assert LR_size == GT_size // scale, f"GT size {GT_size} does not match LR size {LR_size} w.r.t scale of {scale}"
        ...
        elif LR_size is not None or self.opt["phase"]== "test":
            H, W, C = img_LR.shape
            assert LR_size == GT_size // scale, f"GT size {GT_size} does not match LR size {LR_size} w.r.t scale of {scale}"

            if (LR_size < H and LR_size < W) or (self.opt["phase"]== "test"):
                # center crop
                rnd_h = H // 2 - LR_size//2
                rnd_w = W // 2 - LR_size//2
                img_LR = img_LR[rnd_h : rnd_h + LR_size, rnd_w : rnd_w + LR_size, :]
